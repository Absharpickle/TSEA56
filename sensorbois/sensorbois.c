...
#define LINE0_THRESHOLD    180
#define LINE1_THRESHOLD    180
...
static uint8_t ir_to_cm(uint16_t adc) {
    static const uint16_t table[][2] = {
        {720, 10}, {600, 15}, {500, 20}, {400, 25},
        {330, 30}, {270, 40}, {220, 50}, {185, 60},
        {160, 70}, {140, 80}
    };
...
ISR(TIMER0_COMPA_vect) {
    mux_ch = 0; adc_phase = 0; sweep_stage = 0;
    mux_select(0);
...
int main(void) {
    ...
    while (1) {
        if (sweep_done) {
            ...
            //int16_t omega_filt = gyro_lpf_update(omega);
