...
/* Feature-detektion: symmetribaserad klassificering */
#define TIME_WINDOW_TICKS     30
#define MARKER_MIN_TOTAL      5    /* Minsta totala svarta för att överväga markering */
#define SYMMETRY_MIN_PER_SIDE 3    /* Min svarta per sida -> korsning */
#define ASYMMETRY_MIN_DIFF    3    /* Min skillnad mellan sidor -> pickup */
...
/* Sticky-flagga: sätts vid ny feature, nollställs när master läst */
static volatile uint8_t feature_new = 0;

/* Debug/Watch-variabler */
static volatile int16_t line0_dev_dbg = 0;
static volatile int16_t line1_dev_dbg = 0;
static volatile int16_t line1_8_dev_dbg = 0;
...
/* --------------------------------------------------------------------------
 * Korsning eller upphämtning – symmetribaserad
 * -------------------------------------------------------------------------- */
...
static line_feature_t detect_line_feature(const uint16_t *lpf, uint16_t threshold) {
    ...
    uint8_t left_count  = 0; /* index 1..5  */
    uint8_t right_count = 0; /* index 7..11 */
    ...
    for (uint8_t i = 1; i < NUM_LINE_SENSORS; i++) {
        if (lpf[i] > threshold) {
            total_count++;
            if (i <= 5)       left_count++;
            else if (i >= 7)  right_count++;
        }
    }

    black_count_dbg = total_count;
    line_feature_t current = FEATURE_NONE;

    if (total_count >= MARKER_MIN_TOTAL) {
        if (left_count >= SYMMETRY_MIN_PER_SIDE && right_count >= SYMMETRY_MIN_PER_SIDE) {
            current = FEATURE_INTERSECTION;
        } else {
            uint8_t diff = (left_count > right_count) ? (left_count - right_count) : (right_count - left_count);
...
static int16_t gyro_lpf_update(int16_t x) {
    if (!gyro_lpf_init) {
        ...
        gyro_acc = ((int32_t)(IIR_SCALE - GYRO_IIR_ALPHA_NUM) * gyro_acc + 
                    (int32_t)GYRO_IIR_ALPHA_NUM * ((int32_t)x * IIR_SCALE)) / IIR_SCALE;
    }
...
int main(void) {
    ...
    while (1) {
        if (sweep_done) {
            ...
            /* Clampning och konvertering */
            int16_t clamped_angle = angle;
            if (clamped_angle >  127) clamped_angle =  127;
            if (clamped_angle < -127) clamped_angle = -127;
            int8_t angle_8 = (int8_t)clamped_angle;

            /* Status-byte konstruktion */
            uint8_t stat = 0;
            ...
            if (feature_new)         stat |= (1 << 5);
            stat_dbg = stat;

            if (feature != FEATURE_NONE) dev1 = 0;
            
            int16_t clamped_line = dev1;
            if (clamped_line >  127) clamped_line =  127;
            if (clamped_line < -127) clamped_line = -127;
            int8_t dev1_8bit = (int8_t)clamped_line;

            /* Uppdatera sändbuffert (atomärt) */
            cli();
            tx_buf[0] = stat;
            tx_buf[1] = dev1_8bit;
            tx_buf[2] = angle_8;
