...
int main() {
    ...
    uint8_t out_state = 1; // 1 = Auto
    char out_cmd = 'h';    // 'h' = Hjul, 'a' = Arm
    char out_action = 's'; // 'f', 'b', 'l', 'r', 'p', 'd'
    ...
    while (!uppdrag_klart) {
        ...
        if (read(file, buffer_in, 8) == 8) {
            ...
            // Räkna ut felet (begränsat till -128 till +127 för att inte rulla över)
            ...
            if (korsning_detekterad && !var_i_korsning_forra_loopen && !roterar_just_nu) {
                ...
                if (beslut == 'X') { 
                    out_cmd = 'a'; // Vi vill styra armen
                    if (aktuell_rutt == malbeslut) {
                        out_action = 'p'; // Plocka
                        printf("\n*** FRAMME VID NOD %d! PLOCKAR VARA (p)... ***\n", nuvarande_nod);
                        aktuell_rutt = slutbeslut;
                        beslut_index = 0;
                    } else {
                        out_action = 'd'; // Lämna
                        printf("\n*** END NÅDD (NOD %d)! LÄMNAR VARA (d)... ***\n", nuvarande_nod);
                        uppdrag_klart = true;
                    }
                } else {
                    printf("[KORSNING: NOD %d] Skickar hjul-kommando: '%c'\n", nuvarande_nod, beslut);
                    beslut_index++;
                    out_cmd = 'h'; 
                    out_action = beslut;
                    
                    if (beslut == 'l' || beslut == 'r' || beslut == 'b') {
                        roterar_just_nu = true; 
                        rotation_timer = 30; // 1.5 sek blindtid
                    }
                }
            }
            ...
            // --- PACKA PAKETET EXAKT ENLIGT DINA INSTRUKTIONER ---
            buffer_out[0] = 0x05;                             // Start
            buffer_out[1] = out_state;                        // State (1=Auto)
            buffer_out[2] = (uint8_t)out_cmd;                 // Cmd ('h' eller 'a')
            buffer_out[3] = (uint8_t)out_action;              // Action ('f', 'r', 'p' etc)
            buffer_out[4] = out_linjefel_offset;              // Linjefel (128 = 0)
            buffer_out[5] = (uint8_t)(ut_gyro_data & 0xFF);   // Gyro LSB
            buffer_out[6] = (uint8_t)((ut_gyro_data >> 8) & 0xFF); // Gyro MSB
            buffer_out[7] = 0xFF;                             // Stopp

            // --- SKICKA TILL STYRMODUL ---
